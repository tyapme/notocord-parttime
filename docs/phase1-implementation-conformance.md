...
- Fix:
  - 日付（ブラウザ標準 `date` ピッカー）
  - 開始/終了時刻（手入力可）
  - 開始/終了時刻は横並び（`grid-cols-2`）で、入力は `max-w-[10.5rem]` のコンパクト幅 + `min-w-0` ではみ出し防止
  - メッセージ（任意）
- Flex:
...
- 詳細:
  ...
  - 2026-02-28 UI調整: `編集する` / `変更して再申請` を主操作として青ボタンで強調
  - Fix編集時の日付はブラウザ標準 `date` ピッカー
...
- モーダル処理:
  ...
  - Fix変更承認:
    - 日付は単一入力（開始日/終了日の2項目は廃止）
    - 変更理由必須
    - 申請時間帯をはみ出す変更で連絡推奨警告
    - 日付はブラウザ標準 `date` ピッカー、開始/終了時刻は横並び（`grid-cols-2`） + コンパクト幅
  - Flex変更承認:
    - 時間増加時に連絡推奨警告
- 確定済みの再処理:
  - 編集して再確定可能
...
- 2026-02-28 UI調整:
  - ユーザー一覧カードとヘッダーでアルバイト名を表示
  - ヘッダー右上アバターメニューを開いた際は、メールの上にユーザー名を表示
  - 検索入力のアイコンとプレースホルダーが重ならないよう左余白を調整
...
- 出勤警告ドロワー:
  - 警告文言: 「このまま出勤してもよろしいですか？」
- 勤怠修正モーダル:
  - 開始時刻/終了時刻を横並び（`grid-cols-2`）にし、入力を `w-full min-w-0` で崩れ防止
- 勤怠詳細モーダル/ドロワー:
  - ヘッダーの×ボタンとフッターの「閉じる」ボタンは表示しない（背景タップ/Escで閉じる）
- トースト表示:
  - モバイルは下部タブメニューの上（`bottom: calc(env(safe-area-inset-bottom) + 76px)`）に表示
